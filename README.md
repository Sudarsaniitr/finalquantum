...
│
└── results/
    ├── 01_n_copies_effect.png        — swap kernel n=1,2,3 (hardware left | simulator right)
    ├── 02_helstrom_equivalence.png   — swap kernel = Helstrom measurement
    ├── 03_shots_comparison.png       — 256 vs 1024 shots vs theory
    ├── 04_vce_novelty.png            — physical n=3 vs virtual n=3 (VCE)
    ├── hardware_results.json         — hardware kernel values, metrics, shots data
    ├── simulator_results.json        — simulator kernel values, metrics, shots data
    ├── vce_summary.json              — VCE: physical vs virtual n=3, both backends
    └── no_hardware/                  — identical set generated by run_no_hardware.py
        ├── 01_n_copies_effect.png    — single-panel (simulator only)
